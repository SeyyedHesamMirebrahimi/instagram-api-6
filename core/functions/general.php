...
function httpRequest($url, $method = 'GET', array $params = [])
{
    ...
    /*
    if ($GLOBALS['proxy']) {
        curl_setopt($ch, CURLOPT_PROXY, $GLOBALS['proxy']['address'] . '' . $GLOBALS['proxy']['port']);
        if ($GLOBALS['proxy']['auth']) {
            curl_setopt($ch, CURLOPT_PROXYUSERPWD, $GLOBALS['proxy']['auth']['user'] .':' . $GLOBALS['proxy']['auth']['pass']);
        }
...
function colorful_number_icon($number, $tooltip = false) {
    ...
    if($number > 0) {
        return '<span ' . $tooltip_html . ' style="color: #28a745 !important;"><i class="fa fa-fw fa-arrow-up"></i></span>';
    }
    elseif($number < 0) {
        return '<span ' . $tooltip_html . ' style="color: #dc3545 !important;"><i class="fa fa-fw fa-arrow-down"></i></span>';
