        ...
        if($source_account_new->is_private) {
            ...
            if ($media_response && !empty($media_response)) {
                foreach ($media_response['medias'] as $media) {
                    if($media->getLikesCount() > $more_likes){
                        $more_likes = $media->getLikesCount();
                        $background_image = $media->getImageHighResolutionUrl();
                    }
            ...
            $details['background_image'] = $background_image;
...
if(!$source_account->is_private) {
    ...
    $best_time['labels'] = ['12-1 AM', '1-2 AM', '2-3 AM','3-4 AM','4-5 AM','5-6 AM','6-7 AM','7-8 AM','8-9 AM','9-10 AM','10-11 AM', '11-12 AM', 
    ...
    for($i = 0; $i < count($media_results); $i++) {
        ...
        $hou = (new \DateTime())->setTimestamp($media_results[$i]->created_date)->format('H');
        $hou = $hou+0;
        
        if(array_key_exists($hou, $best_time['data'])){
            $best_time['data'][$hou]['count'] = $best_time['data'][$hou]['count'] + 1;
            $best_time['data'][$hou]['likes'] = $best_time['data'][$hou]['likes'] + $media_results[$i]->likes;
        }else{
            $best_time['data'][$hou]['count'] = 1;
            $best_time['data'][$hou]['likes'] = $media_results[$i]->likes; 
        }
    ...
    foreach ($best_time['labels'] as $key => $value) {
        $best_time_to_post['labels'][$key] = $value;
        if(isset($best_time['data'][$key])){
            $best_time_to_post['data'][$key] = intval($best_time['data'][$key]['likes']/$best_time['data'][$key]['count']);
        }else{
            $best_time_to_post['data'][$key] = 0;
        }
    }
    ...
    $best_time_to_post = generate_chart_data($best_time_to_post);
}
