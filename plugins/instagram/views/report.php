...
*/ ?>    
    ...
    <div class="margin-bottom-6">
        ...
        <p class="text-muted">Each value in this chart is equal to the Average Engagement Rate of the Post created in that specific time.</p>

        <div class="chart-container">
            <canvas id="best_time_to_post"></canvas>
        </div>

    </div>
    
    
    <?php if(count((array) $source_account->details->top_posts_by_likes) > 0): ?>
    <div class="margin-bottom-6 d-print-none">
        ...
        <div class="row mb-5">
            <?php foreach($source_account->details->top_posts_by_likes as $shortcode => $engagement_rate): ?>
    ...
    <?php if(count((array) $source_account->details->top_posts_by_comments) > 0): ?>
    <div class="margin-bottom-6 d-print-none">
        ...
        <div class="row mb-5">
            <?php foreach($source_account->details->top_posts_by_comments as $shortcode => $engagement_rate): ?>
                    ...
                    <div class="d-flex flex-column">
                        <?php foreach((array) $source_account->details->top_mentions as $mention => $use): ?>
                            <div class="d-flex align-items-center">
                                ...
                                <a href="https://www.instagram.com/<?= $mention ?>" class="report-content-number-link" target="_blank" style="color:white"><?= $mention ?></a>
                    ...
                    <div class="d-flex flex-column">
                        <?php foreach((array) $source_account->details->top_hashtags as $hashtag => $use): ?>
                            <div class="d-flex align-items-center">
                                ...
                                <a href="https://www.instagram.com/explore/tags/<?= $hashtag ?>/" class="report-content-number-link" target="_blank" style="color:white">#<?= $hashtag ?></a>
        ...
        <div class="margin-bottom-6">
            ...
            <table class="table table-responsive-md">
                ...
                <?php foreach($media_results as $media): ?>
                    <tr>
                        <td>
                            <a href="<?= $media->media_url ?>" target="_blank" data-toggle="tooltip" style="color:white" title="<?= $language->instagram->report->display->media_type->{strtolower($media->type)} ?>">
                                <?php
                                switch($media->type) {
                                    case 'IMAGE':
                        ...
                        <td style="color:white"><span data-toggle="tooltip" title="<?= (new \DateTime())->setTimestamp($media->created_date)->format($language->global->date->datetime_format . ' H:i:s') ?>"><?= (new \DateTime())->setTimestamp($media->created_date)->format($language->global->date->datetime_format) ?></span></td>
                        <td style="color:white"><span data-toggle="tooltip" title="<?= $media->caption ?>"><?= string_resize($media->caption, 25) ?></span></td>
...
<script>
    ...
    ///////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////////////////////////////////////////////
    let best_time_to_post = document.getElementById('best_time_to_post').getContext('2d');

    let gradient = best_time_to_post.createLinearGradient(237, 255, 7, 0.7);
    gradient.addColorStop(0, 'rgba(237, 255, 7, 0.6)');
    gradient.addColorStop(1, 'rgba(237, 255, 7, 0.5)');

    new Chart(best_time_to_post, {
        type: 'bar',
        data: {
            labels: <?= $best_time_to_post['labels'] ?>,
            datasets: [{
                label: 'Best Time To Post Chart',
                data: <?= $best_time_to_post['data'] ?>,
