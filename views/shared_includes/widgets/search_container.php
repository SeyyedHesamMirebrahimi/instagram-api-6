...
<div class="search-container d-flex flex-column align-items-center justify-content-center container" style="margin-top: 80px;">

    <h3 class="font-weight-bolder text-dark mb-5" style="color:black"> Try it now <?php //$language->global->menu->search_title ?></h3>
    <form class="form-inline d-inline-flex justify-content-center search_form" action="index" method="POST">
        ...
        <div class="dropdown my-2 mr-2">
            ...
            <div class="dropdown-menu">
                <?php foreach($sources as $source): ?>
                    <a class="dropdown-item source-select-item" href="#" data-source="<?= $source ?>"><i class="fab fa-fw fa-<?= $source ?>"></i> <?= ucfirst($source) ?></a>
                <?php endforeach ?>
            </div>
        </div>
        <?php endif ?>
        ...
        <div class="my-2 index-input-div">
            <i class="fa fa-fw fa-search text-black-50 index-search-input-icon"></i>
            <input class="form-control mr-2 index-search-input border-0 form-control-lg source_search_input" name="query" type="text" placeholder="<?= $language->global->menu->search_placeholder ?>" aria-label="<?= $language->global->menu->search_placeholder ?>">
        </div>

        <button type="submit" class="btn btn-<?= reset($sources) ?> index-submit-button border-0 d-inline-block my-2"><i class="fa fa-spin fa-refresh button-spinner button-search-spinner" style="display:none"></i>&nbsp;<?= $language->global->search ?></button>
        <script>
        $(function() {
            $('.search_form').on('submit', (event) => {
            	let button = $(".index-submit-button");
            ...
            <?php foreach($plugins->plugins as $plugin_identifier => $value): ?>
                <?php if($plugins->exists_and_active($plugin_identifier)): ?>
                    case '<?= $plugin_identifier ?>':
                        $this.closest('form').find('.source_search_input').attr('placeholder', '<?= $language->{$plugin_identifier}->global->search_placeholder ?>');
                        break;
                <?php endif ?>
            <?php endforeach ?>
